...
from ..services.convert_meta import load_meta, meta_matches, save_meta
...
def _wait_for_readable_source(path: Path, attempts: int = 8, delay_seconds: float = 0.35) -> None:
    """Best-effort wait when another app may still have the file open."""
...
def _convert_to_argument(source: Path, options: ConversionOptions) -> str:
    suffix = source.suffix.lower()
    if suffix == ".pptx":
        props: dict[str, object] = dict(options.impress_pdf_extra_properties)
        props.setdefault("ExportNotesPages", options.export_notes_pages)
        return f"pdf:impress_pdf_Export:{json.dumps(props, separators=(',', ':'))}"
    return "pdf"
...
class LibreOfficeStrategy:
    ...
    def convert(self, item: QueueItem, options: ConversionOptions | None = None) -> ConversionResult:
        opts = options or ConversionOptions(timeout_seconds=self.timeout_seconds)
        timeout = opts.timeout_seconds if opts.timeout_seconds > 0 else self.timeout_seconds
        ...
        try:
            _wait_for_readable_source(item.source_path)
        except OSError as exc:
            return ConversionResult(False, None, f"Could not read source file (still open?): {exc}")
        ...
        try:
            ...
            return ConversionResult(False, None, f"Could not stat source file: {exc}")

        source_sha256 = item.content_sha256 or sha256_file(item.source_path)
        item.content_sha256 = source_sha256

        if opts.skip_if_unchanged and output_pdf.exists() and output_pdf.stat().st_size > 0:
            existing_meta = load_meta(output_pdf)
            if existing_meta and meta_matches(existing_meta, source_sha256, source_size):
                return ConversionResult(True, output_pdf, "Skipped unchanged output (hash matches).")

        convert_to = _convert_to_argument(item.source_path, opts)
        command = [
        ...
        save_meta(output_pdf, source_sha256, source_size)
