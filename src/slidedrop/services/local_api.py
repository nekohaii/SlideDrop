...
def serve_convert_api(host: str = "127.0.0.1", port: int = 8765) -> None:
    server = LocalConvertHTTPServer((host, port), Handler)
    print(f"SlideDrop local API listening on http://{host}:{port} (POST /v1/convert, GET /health)")
